...
void conv_forward(conv_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
  int x, y;
  int ox, oy;
  int f_depth, V_depth, f_sx, f_sy;
  int l_out_sy, l_out_sx;
  int f_loc, v_loc;
  int l_out_depth;
  double a;
  double l_bias_wd;
              ...
              for(int fy = 0; fy < f_sy; fy++) {
                ...
                if(oy >= 0) {
                  ...
                  if (ox >= 0 && ox + 4 < V_sx) {
                    ...
                    f_loc = ((f_sx * fy))*3;
                    v_loc = ((V_sx * oy)+ox)*V_depth;
                    return_v = _mm256_setzero_pd();

                    next_v = _mm256_loadu_pd((const double*) &V->w[v_loc]);
                    next_f = _mm256_loadu_pd((const double*) &f->w[f_loc]);
                    return_v = _mm256_add_pd(return_v, _mm256_mul_pd(next_f, next_v));

                    next_v = _mm256_loadu_pd((const double*) &V->w[v_loc+4]);
                    next_f = _mm256_loadu_pd((const double*) &f->w[f_loc+4]);
                    return_v = _mm256_add_pd(return_v, _mm256_mul_pd(next_f, next_v));

                    next_v = _mm256_loadu_pd((const double*) &V->w[v_loc+8]);
                    next_f = _mm256_loadu_pd((const double*) &f->w[f_loc+8]);
                    return_v = _mm256_add_pd(return_v, _mm256_mul_pd(next_f, next_v));

                    _mm256_store_pd(partial, return_v);
                    a = a + partial[0] + partial[1] + partial[2] + partial[3];

                    a += f->w[f_loc+12] * V->w[v_loc+12];
                    a += f->w[f_loc+13] * V->w[v_loc+13];
                    a += f->w[f_loc+14] * V->w[v_loc+14];
                    ...
                    a += f->w[f_loc+8] * V->w[v_loc+8];
                    if (ox + 3 >= V_sx) {
                      continue;
                    }
                    a += f->w[f_loc+9] * V->w[v_loc+9];
                    a += f->w[f_loc+10] * V->w[v_loc+10];
                    a += f->w[f_loc+11] * V->w[v_loc+11];
                    ...
                    if (ox >= 0) {
                      f_loc = ((f_sx * fy))*3;
                      v_loc = ((V_sx * oy)+ox)*V_depth;
                      a += f->w[f_loc] * V->w[v_loc];
                      a += f->w[f_loc+1] * V->w[v_loc+1];
                      a += f->w[f_loc+2] * V->w[v_loc+2];
                    }
                ...
                if(oy >= 0) {
                  ...
                  for(int fx = 0; fx < f_sx; fx++) {
                    ...
                    if (ox >=0) {
                        ...
                        next_v = _mm256_loadu_pd((const double*) &V->w[v_loc+12]);
                        next_f = _mm256_loadu_pd((const double*) &f->w[f_loc+12]);
                        return_v = _mm256_add_pd(return_v, _mm256_mul_pd(next_f, next_v));
              ...
              if(oy >= 0) {
                ...
                for(int fx = 0; fx < f_sx; fx++) {
                  ...
                  if (ox >= 0) {
                    ...
                    next_v = _mm256_loadu_pd((const double*) &V->w[v_loc+16]);
                    next_f = _mm256_loadu_pd((const double*) &f->w[f_loc+16]);
                    return_v = _mm256_add_pd(return_v, _mm256_mul_pd(next_f, next_v));
...
void pool_forward(pool_layer_t* l, vol_t** in, vol_t** out, int start, int end) {
  int x, y;
  int l_out_depth, l_stride, l_sx, l_sy;
  int V_sy, V_sx;
  int A_sx, A_depth;
  double a;
  for (int i = start; i <= end; i++) {
    ...
    for(int d=0;d<l->out_depth;d++) {
      ...
      l_sx = l->sx;
      l_sy = l->sy;
      for(int ax=0; ax<l->out_sx; x+=l_stride,ax++) {
        y = -l->pad;
        for(int ay=0; ay<l->out_sy; y+=l_stride,ay++) {
...
void net_classify_cats(network_t* net, vol_t** input, double* output, int n) {
  ...
  #pragma omp parallel
  {
    ...
    for (int i = 0; i < n/8*8; i+=8) {
      ...
      batch_t b_zero = batch[0];

      copy_vol(b_zero[0], input[i]);
      copy_vol(b_zero[1], input[i+1]);
      copy_vol(b_zero[2], input[i+2]);
      copy_vol(b_zero[3], input[i+3]);
      copy_vol(b_zero[4], input[i+4]);
      copy_vol(b_zero[5], input[i+5]);
      copy_vol(b_zero[6], input[i+6]);
      copy_vol(b_zero[7], input[i+7]);

      net_forward(net, batch, 0, 7);

      batch_t b_eleven = batch[11];
      
      output[i] = b_eleven[0]->w[CAT_LABEL];
      output[i+1] = b_eleven[1]->w[CAT_LABEL];
      output[i+2] = b_eleven[2]->w[CAT_LABEL];
      output[i+3] = b_eleven[3]->w[CAT_LABEL];
      output[i+4] = b_eleven[4]->w[CAT_LABEL];
      output[i+5] = b_eleven[5]->w[CAT_LABEL];
      output[i+6] = b_eleven[6]->w[CAT_LABEL];
      output[i+7] = b_eleven[7]->w[CAT_LABEL];
    }
