...
class CNNClassifier(Classifier):
  ...
  def backward(self, data, count):
    ...
    L_partials = softmax.map(lambda (x, l, (L, df)): L)
    L = L_partials.reduce(lambda L1, L2: L1 + L2)
    ...
    dLdA10, dLdb10 = dLdA_dLdb_10.map(lambda (x, l, (dx, dA, db)): (dA, db)).reduce(lambda d1, d2: (d1[0] + d2[0], d1[1] + d2[1]))
    ...
    dLdA7, dLdb7 = dLdA_dLdb_7.map(lambda (x, l, (dx, dA, db)): (dA, db)).reduce(lambda d1, d2: (d1[0] + d2[0], d1[1] + d2[1]))
    ...
    dLdA4, dLdb4 = dLdA_dLdb_4.map(lambda (x, l, (dx, dA, db)): (dA, db)).reduce(lambda d1, d2: (d1[0] + d2[0], d1[1] + d2[1]))
