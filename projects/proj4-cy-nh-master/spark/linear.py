...
class LinearClassifier(Classifier):
  ...
  def backward(self, data, count):
    ...
    """
    L = softmax.map(lambda (x, (L, df)): L).reduce(lambda L1, L2: L1 + L2)
    ...
    dLdA_dLdb = softmax.map(lambda (x, (L, df)): linear_backward(df, x, self.A))
    dLdA, dLdb = dLdA_dLdb.map(lambda (dx, dA, db): (dA, db)).reduce(lambda d1, d2: (d1[0] + d2[0], d1[1] + d2[1]))
