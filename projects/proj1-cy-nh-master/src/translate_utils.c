...
int translate_num(long int* output, const char* str, long int lower_bound, 
    ...
    if (!str || !output) {
      ...
      char** str_end = (char**) calloc(1, sizeof(char));
      long int str_as_long = strtol(str, str_end, 0);
      ...
      free(str_end);
      // Check if the number was within bounds
      if (str_as_long >= lower_bound && str_as_long <= upper_bound) {
        *output = str_as_long;
        return 0;
      } else {
        return -1;
