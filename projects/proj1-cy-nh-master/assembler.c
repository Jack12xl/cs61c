...
int pass_one(FILE* input, FILE* output, SymbolTable* symtbl) {
    if (!input || !output) {
        fprintf(stdout, "I/O error occured on pass one. Null file pointer encountered.\n");
        return -1;
    }
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        input_line_number += 1;
        // Truncate any comments in the line.
        skip_comment(buf);

        // Next, use strtok() to scan for the first token. If there's nothing,
        // go to the next line.
        char* first_token;
        first_token = strtok(buf, IGNORE_CHARS);


        // If the line is not empty:
        if (first_token != NULL) {
            int label_err;
            // If the first token is a label, map it to the next byte_offset in symtbl.
            label_err = add_if_label(input_line_number, first_token, byte_offset, symtbl);
            char* name;

            if (label_err == 1) {
                // first_token was a label and was added correctly. Assign name
                // to the next token.
                name = strtok(NULL, IGNORE_CHARS);

            } else if (label_err == -1) {
                // An error occurred in adding first_token as a label.
                // raise_label_error(input_line_number, first_token);
                error_occurred = -1;
                name = strtok(NULL, IGNORE_CHARS);
            } else {
                // first_token was not a label; can now assume it is a name.
                name = first_token;
            }
            ...
            if (name != NULL) {
                ...
                char* empty = strtok(NULL, IGNORE_CHARS);
                if (empty != NULL) {
                    raise_extra_arg_error(input_line_number, empty);
                }
                ...
                // raise_inst_error().
                int current_error;
                current_error = write_pass_one(output, name, args, num_args);
                byte_offset += (4 * current_error);

                if (!current_error) {
                    error_occurred = -1;
                    raise_inst_error(input_line_number, name, args, num_args);
                }
            }
        }
...
int pass_two(FILE *input, FILE* output, SymbolTable* symtbl, SymbolTable* reltbl) {
    if (!input || !output) {
        fprintf(stdout, "I/O error occured on pass two. Null file pointer encountered.\n");
        return -1;
    }
    int error_occurred = 0;
    ...
    while (fgets(buf, BUF_SIZE, input)) {
        ...
        if (name != NULL) {
            ...
            // raise_inst_error().
            int current_error;
            current_error = translate_inst(output, name, args, num_args, byte_offset,
                symtbl, reltbl);
            byte_offset += 4;
